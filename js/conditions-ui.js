...
(function(){
  ...
  function enhance(list){
    ...
    const initialCount=10,hiddenButtons=buttons.slice(initialCount);if(hiddenButtons.length){hiddenButtons.forEach(button=>button.hidden=true);const more=document.createElement('button');more.type='button';more.className='cm-more-spots';more.innerHTML=`残り${hiddenButtons.length}地点を表示 <b>⌄</b>`;more.addEventListener('click',()=>{const opening=more.dataset.open!=='true';hiddenButtons.forEach(button=>button.hidden=!opening);more.dataset.open=opening?'true':'false';more.innerHTML=opening?`残り${hiddenButtons.length}地点を閉じる <b>⌃</b>`:`残り${hiddenButtons.length}地点を表示 <b>⌄</b>`});list.appendChild(more)}
    const moveDetail=()=>{const selected=buttons.find(button=>button.classList.contains('selected')),detail=list.parentElement.querySelector('.conditions-map-detail');if(!selected||!detail)return;selected.hidden=false;selected.after(detail);requestAnimationFrame(()=>selected.scrollIntoView({behavior:'smooth',block:'start'}))};
