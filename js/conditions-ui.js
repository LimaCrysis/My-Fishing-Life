...
(function(){
  ...
  function enhance(list){
    ...
    const moveDetail=()=>{const selected=buttons.find(button=>button.classList.contains('selected')),detail=list.parentElement.querySelector('.conditions-map-detail');if(!selected||!detail)return;selected.hidden=false;selected.after(detail)};
