...
(function(){
  'use strict';
  const URLS={baselines:'./data/fishing-forecast-baselines.json',spots:'./data/fishing-forecast-spots.json',expansion:'./data/fishing-forecast-expansion.json'};
  ...
  function loadConfig(){return configPromise||(Promise.all([fetchJSON(URLS.baselines),fetchJSON(URLS.spots),fetchJSON(URLS.expansion)]).then(([baselines,spots,expansion])=>{const merged=withExpansion(baselines,spots,expansion);return window.MFLForecastValidation.validate(merged.baselines,merged.spots)}).catch(error=>{configPromise=null;throw error}))}
